...
void design_bandstop_filter(long double *fir_filter,int n, float f1, float f2)
{
	...
	{
		if(i==m)
			fir_filter[i]=(0.54-(0.46*(cos(2*M_PI*i/(n-1)))))*(1-(2*(f2-f1)));
		else
			fir_filter[i]=((0.54-(0.46*(cos(2*M_PI*i/(n-1)))))*((sin(f1*2*M_PI*(i-m))-sin(f2*2*M_PI*(i-m)))/(M_PI*(i-m))));
	}
}
...
int main(int argc, char *argv[])
{
	...
	design_bandstop_filter(fir_filter,taps,cutoff_freq1/(sampling_freq),cutoff_freq2/(sampling_freq));
